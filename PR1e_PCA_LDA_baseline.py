...
def main():	
	...
	for method in methods:
		...
		y3 = mAP_pca_list_norm
		y4 = mAP_lda_list_norm 
				
		plt.figure(figsize=(10,10))
		plt.plot(x1, y1, color = 'red', label = 'PCA Original[O]', marker = 'o')
		plt.plot(x1, y2, color = 'green', label = 'PCA-LDA Original[O]', marker = 'o')	
		plt.plot(x1, y3, color = 'red', label = 'PCA Norm[N]', marker = 'x')
		plt.plot(x1, y4, color = 'green', label = 'PCA-LDA Norm[N]', marker = 'x')	
		plt.grid(color = 'black', linestyle = '-', linewidth = 0.1)			# parameters for plot grid
		title_name = str(method_name[method_count]+" "+test_name[name_count]+' Baseline PCA and PCA-LDA Performance')
		...
		for i, txt in enumerate(y2):
			plt.annotate(txt, (x1[i], y2[i]))
		for i, txt in enumerate(y2):
			plt.annotate(txt, (x1[i], y3[i]))
		for i, txt in enumerate(y2):
			plt.annotate(txt, (x1[i], y4[i]))
		'''
	...
	for method in methods:
		...
		for test_data in test_datas:
			...
			print(method_name[method_count],test_name[name_count],"mAP:",method_map)
			name_count = name_count + 1
		print("		")
		method_count = method_count + 1
