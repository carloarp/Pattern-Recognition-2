...
def metrics(nb, Xtest,ytest, rank=320):
    rank_per_image = []
    recall_per_image = []
    # rank = 200
    interp_per_image = []
    for x in range(0,32):
        ...
        for i in range(1,rank):
            ...
            if re in interp_r:
                ...
                interp_r[re] = pr
            rank_acc.append(min(correct,1))
# 
        interp_per_image.append(interp_r)
        rank_per_image.append(rank_acc)

    rank_per_image = np.asarray(rank_per_image)
    avg_prec_per_image = []
    avg_prec_per_image_1 = []

    for dic in interp_per_image:
        x = np.arange(0, 1.1, 0.1)
      
        # avg_prec_per_image.append(sum([k*v for k,v in dic.items()])/10)
        local = []
        for vals in x:
            current_precision = 0
            for key in list(dic.keys()):
                if key >= vals:
                    current_precision = max(current_precision, dic[key])
            local.append(current_precision)
        avg_prec_per_image_1.append(sum(local)/11)

        avg_prec_per_image.append(sum([max(list(dic.values())[i:]) for i in range(len(dic.items()))])/10)
    avg_acc_per_rank = np.mean(rank_per_image, axis=0)
    avg_prec_per_image = np.asarray(avg_prec_per_image)
    return  np.mean(avg_prec_per_image_1), np.mean(avg_prec_per_image), avg_acc_per_rank[0], avg_acc_per_rank[9]
