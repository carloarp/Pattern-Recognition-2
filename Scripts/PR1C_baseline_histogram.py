...
def main():		
	...
	for bin_width in range(10,21,2):
	
		#bin_width = 10
		intensity_max = 255
		n_bins = math.ceil(intensity_max/bin_width)
		
		bin_list = np.arange(0, 270, bin_width).tolist()	# Create a bin list from 0-260
		...
		X_hist = []
		for i in range(0,X_test.shape[1]):
			X_hist_test, bins, patches = plt.hist(A_test_original[i], bins = bin_list)	# This is already normalised when using the function
		...
		for method in methods:
			...
			print("#Bins:",n_bins,",",method_name[method_count],"mAP:",method_map,",Acc@rank1:",acc1,",Acc@rank10:",acc10)
